...
const createProduct = async function (req, res) {
  try {
    ...
    if (uniqueTitle) {
      return res
        .status(400)
        .send({ status: false, message: "This product is already available" });
    }
    ...
    if (!validator.isValidNumber(price))
      return res
        .status(400)
        .send({ status: false, message: "Please enter price in only Number" });
    ...
    if (style) {
      ...
      if (!validator.isValidTitle(style))
        return res
          .status(400)
          .send({ status: false, message: "Please enter style name in alpha" });
    }
    ...
    if (availableSize.length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "Please provide product sizes" });
    }
...
const getProductByFilter = async (req, res) => {
  ...
  if (!productData) {
    return res
      .status(404)
      .send({ status: false, message: "No product found with these details" });
  }
  let sortProduct = "";
  if (priceSorts == 1) {
    sortProduct = productData.sort((a, b) => a.price - b.price);
  }
  if (priceSorts == -1) {
    sortProduct = productData.sort((a, b) => b.price - a.price);
  }
...
const updateProductDetails = async function (req, res) {
  try {
    ...
    let findProductId = await productModel.findById({
      _id: productId,
      isDeleted: false,
    });
    if (!findProductId)
    ...
    if (image && image.length > 0) {
      if (!validator.isValidFile(image[0].originalname))
        return res
          .status(400)
    ...
    if (title) {
      if (!validator.isValid(title))
        return res
          .status(400)
          .send({ status: false, message: "title Should be Valid" });
      if (!validator.isValidTitle(title))
        return res
          .status(400)
          .send({ status: false, message: "title should not contain number" });
    ...
    if (description) {
      if (!validator.isValid(description))
        return res
          .status(400)
          .send({ status: false, message: "description Should be Valid" });
    }
    ...
    if (price) {
      if (!validator.isValidNumber(price))
        return res
          .status(400)
          .send({ status: false, message: "price Should be Valid" });
    }
    ...
    if (style) {
      ...
      if (!validator.isValidTitle(style))
        return res
          .status(400)
          .send({ status: false, message: "style Should Not Contain Numbers" });
    }
    ...
    const updateDetails = await productModel
      .findOneAndUpdate({ id: productId, isDeleted: false }, updateData, {
        new: true,
      })
      .select({ _v: 0 });
