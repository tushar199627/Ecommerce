...
let createUser = async (req, res) => {
  try {
    ...
    if (!isValid(email)) {
      return res.status(400).send({
        status: false,
        message: "Please provide a Email Id or a Valid Email Id",
      });
    }
    ...
    if (data.address) {
      try {
        ...
        return res.status(400).send({
          status: false,
          message: "address should be in a valid object format",
        });
      }
      ...
      if (!isValid(add.shipping && add.billing)) {
        return res.status(400).send({
          status: false,
          message: "Please provide shipping Address And Billing Address",
        });
      }
      ...
      if (!isValid(add.shipping.street)) {
        return res.status(400).send({
          status: false,
          message: "Shipping Street should be Present",
        });
      }
      ...
      if (!validCity.test(add.shipping.city)) {
        return res
          .status(400)
          .send({ status: false, message: "Shipping city cannot be Number" });
      }
      ...
      if (!isValid(add.billing.street)) {
        return res
          .status(400)
          .send({ status: false, message: "Billing Street should be Present" });
      }
...
let loginUser = async function (req, res) {
  try {
    ...
    if (!decypt) {
      return res
        .status(400)
        .send({ status: false, message: "Password is inappropriate" });
    }
...
let getUserById = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(userId)) {
      return res
        .status(400)
        .send({ status: false, msg: "userId is not a valid objectId" });
    }
...
let updateUserProfile = async (req, res) => {
  try {
    ...
    if (!isValidRequestBody(dataFromBody) && !files) {
      return res.status(400).send({
        status: false,
        message: "Please provide detail which you want to update",
      });
    }
    ...
    if (data.hasOwnProperty("address")) {
      ...
      if (add.hasOwnProperty("shipping")) {
        ...
        if (!isValid(add.shipping)) {
          return res.status(400).send({
            status: false,
            message: "Please provide Address shipping",
          });
        }
      ...
      if (add.hasOwnProperty("billing")) {
        ...
        if (!isValid(add.billing)) {
          return res
            .status(400)
            .send({ status: false, message: "Please provide Address billing" });
        }
